...
function expand_dgttsp( input )
    ...
    for token in input:gmatch("([^/]+)") do
        ...
        for i = 1, #words do
            ...
            elseif i == #words and abbr_end[lowercaseword] ~= nil then
...
function process_car_reminders( maneuver, useSignpost )
    ...
    if ( maneuver.id == "ENTER_HOV") or ( maneuver.id == "LEAVE_HOV" ) then
        set_result_id( 3 , maneuver_check[ maneuver.id ] )
    end
    maneuver_car( maneuver, useSignpost )
...
function escape_string( text )
    ...
    return (string.gsub(text, magic_character_set, '%%%1'))
end

function escape_replacement_string( text )
...
function init_custom_voice_trigger_table( )
    ...
    cvt_handler_methods["GUIDANCE_INACTIVE"] = function() process_info_message( "GUIDANCE_INACTIVE" ) end
    cvt_handler_methods["ROUTE_SELECTION"] = function() process_info_message( "ROUTE_SELECTION" ) end
    cvt_handler_methods["ROUTE_HOV_ACTIVATED"] = function() process_info_message( "ROUTE_HOV_ACTIVATED" ) end
end
...
-- construct the prompt for information messages
function process_info_message( )
    set_result_id( 1, "x" )
    set_result_id( 2, "i" )
    if ( trigger == "GUIDANCE_INACTIVE" ) then
        set_result_id( 3, "g" )
    elseif ( trigger == "ROUTE_SELECTION" ) then
        set_result_id( 3, "s" )
    elseif ( trigger == "ROUTE_HOV_ACTIVATED" ) then
        set_result_id( 3, "h" )
    end
end
